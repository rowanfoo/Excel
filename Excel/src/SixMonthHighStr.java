...
public class  SixMonthHighStr  extends MyDatabase {
	/**
	 * 
	 * 
	 * 
	 ...
	 */
		...
		private void addRecord(String code,double close,double sixMonth ) throws Exception{
			...
			String mysql1 ="INSERT INTO techstr (code,date ,mode,close,sixMonthPrice) VALUES (?,?,14,?,?)"  ; 
	...
	public void addDataX() throws Exception {
		...
		try (DAOFactoryDataReport dr = new DAOFactoryDataReport()) {
			...
			String mysql = "   select * from "
					+ "  ( SELECT  max(close)as closeM  FROM fortune.data where date between subdate(current_date   , Interval 5 month)  and subdate(current_date   , Interval 1 day)  and code=? ) as t,"
	...
	public void run() {
		try {
			...
			logger.info("SixMonthHighStr run 1");
			...
			System.out.println("SixMonthHighStr Error run " + e);
			logger.severe("SixMonthHighStr Error run " + e);
